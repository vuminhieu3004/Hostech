...
import { Form, Input, InputNumber, Select, Button, notification, Spin, Alert } from "antd";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { useNavigate, useParams } from "react-router";
import { ArrowLeft } from "lucide-react";
import { getContractById, updateContract, mockTenants } from "../../../Api/ContractApi";
        ...
        return (
            <div className="p-6">
                <Alert
                    ...
                    message={is404 ? "Không tìm thấy hợp đồng (404)" : "Không có quyền truy cập (403)"}
                    showIcon
        ...
        <div className="flex flex-col gap-5 p-5">
            ...
            >
                <div className="p-5">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <Form.Item name="status" label="Trạng thái">
                            <Select>
                                ...
                                <Select.Option value={ContractStatus.EXPIRED}>Hết hạn</Select.Option>
                                <Select.Option value={ContractStatus.TERMINATED}>Đã chấm dứt</Select.Option>
                            </Select>
                        </Form.Item>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Form.Item name="rent_price" label="Giá thuê (VNĐ)" rules={[{ required: true }]}>
                            <InputNumber className="w-full" min={0} />
                        </Form.Item>
                        <Form.Item name="deposit_amount" label="Tiền đặt cọc (VNĐ)" rules={[{ required: true }]}>
                            <InputNumber className="w-full" min={0} />
                        </Form.Item>
                    </div>

                    <div className="grid grid-cols-3 gap-4">
                        <Form.Item name="billing_cycle" label="Kỳ thanh toán" rules={[{ required: true }]}>
                            <Select options={billingCycleOptions} />
                        </Form.Item>
                        <Form.Item name="due_day" label="Ngày đến hạn" rules={[{ required: true }]}>
                            <Input placeholder="VD: 5" />
                        </Form.Item>
                        <Form.Item name="cutoff_day" label="Ngày chốt" rules={[{ required: true }]}>
